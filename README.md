...
This repository provides Terraform modules to deploy an ECS-based OpenMetadata service with an Aurora database. Terragrunt configuration is centralized with `infra/terragrunt/root.hcl`, and the *live* directories (`dns`, `backend`, `cicd`, `aws`) simply include this root file for remote state and provider settings. Please note the following when using this module:
...
3. Run `terragrunt plan` inside `infra/terragrunt/live/dev/dns` and then `infra/terragrunt/live/dev/aws`.
